...
- Inline single-field edits go through a `use-<entity>-update` hook (optimistic write → `revalidator.revalidate()` → toast + rethrow on failure), as in `use-task-update.ts` / `use-project-update.ts`.
